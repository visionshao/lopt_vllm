...
from vllm.tokenizers import maybe_get_lopt_tokenizer
import time
...
class InputPreprocessor:
    def __init__(
        ...
        # if self.enable_lopt:
        #     logger.warning(f"Lossless Parallel Tokenizer Enabled! pool size={self.model_config.lopt_pool_size}, chunk length={self.model_config.lopt_chunk_size}.")
        ...
        #     )
        # else:
        #     logger.warning(f"Lossless Parallel Tokenizer is Not Enabled! due to {self.enable_lopt}")
    ...
    def _tokenize_prompt(
        ...
        """
        # tokenizer = self.get_tokenizer() if not self.enable_lopt else self.lopt_tokenizer
        ...
        r = tokenizer.encode(prompt, **tokenization_kwargs)
        end = time.perf_counter()
        elapsed = end - start
        logger.warning(
            "Tokenization time  %.4f seconds (prompt length: %d chars)", elapsed, len(prompt) if prompt else 0
        )
        return r
