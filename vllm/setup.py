...
class cmake_build_ext(build_ext):
    ...
    def _build_lopt_cpp(self) -> None:
        """Build the lopt_cpp extension independently (not through CMake).

        This avoids:
        - Waiting for the full vLLM CMake configure (CUTLASS download, etc.)
        - CMake pybind11/Python include path resolution issues
        - A compile failure blocking the entire vllm build
        """
        lopt_csrc = ROOT_DIR.parent / "benchmarks" / "csrc" / "match_merge.cpp"
        if not lopt_csrc.exists():
            logger.info("lopt_cpp source not found at %s, skipping.", lopt_csrc)
            return

        # Install dir = where vllm/__init__.py lives
        outdir = Path(self.get_ext_fullpath("vllm._C")).parent.absolute()
        ext_suffix = sysconfig.get_config_var("EXT_SUFFIX")
        if not ext_suffix:
            ext_suffix = ".so"
        output_path = outdir / f"lopt_cpp{ext_suffix}"
        ...
        torch_include = subprocess.check_output(
            [sys.executable, "-c", "import os, torch; print(os.path.join(os.path.dirname(torch.__file__), 'include'))"],
            text=True,
        ).strip()

        torch_extra = subprocess.check_output(
            [sys.executable, "-c", "import torch.utils.cpp_extension as e; print(' '.join(e.include_paths()))"],
            text=True,
        ).strip()

        # Also try pip-installed pybind11 if available (fallback)
        try:
        ...
        cmd = [
            ...
            f"-I{py_includes}",
            f"-I{torch_include}",
            *[f"-I{p}" for p in torch_extra.split()],
            *pybind11_include.split(),
            str(lopt_csrc),
            "-o", str(output_path),
        ]
        logger.info("Building lopt_cpp: %s", " ".join(cmd))
        try:
            subprocess.check_call(cmd)
            logger.info("lopt_cpp built successfully: %s", output_path)
        except Exception as e:
            logger.warning(
                "lopt_cpp build failed (%s). "
                "LOPT parallel tokenization will fall back to standard tokenization. "
                "To retry: cd benchmarks/csrc && bash build.sh",
                e,
            )
    ...
    def build_extensions(self) -> None:
        ...
        # Build lopt_cpp separately after the main build
        self._build_lopt_cpp()
